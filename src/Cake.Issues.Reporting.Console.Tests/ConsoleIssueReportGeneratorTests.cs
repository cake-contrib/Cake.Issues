...
public sealed class ConsoleIssueReportGeneratorTests
{
    ...
    public sealed class TheInternalCreateReportMethod
    {
        ...
        public sealed class WithShowDiagnosticsEnabled(ITestOutputHelper output)
        {
            ...
            [Fact]
            public void Should_Filter_Issues_Without_Line()
            {
                // Given
                using var tempSourceFile = new TemporarySourceFile("Testfiles.TestFile.txt");
                var filePath = tempSourceFile.FilePath;
                ...
                // Then
                fixture.Log.Entries.ShouldContain(x => x.Message == "1 issue(s) were filtered because they don't contain line or column information.");
            }

            [Fact]
            public void Should_Filter_Issues_Without_Column()
            {
                // Given
                using var tempSourceFile = new TemporarySourceFile("Testfiles.TestFile.txt");
                var filePath = tempSourceFile.FilePath;
            ...
            [Fact]
            public void Should_Not_Filter_Issues_With_Line_And_Column()
            {
                // Given
                using var tempSourceFile = new TemporarySourceFile("Testfiles.TestFile.txt");
                var filePath = tempSourceFile.FilePath;
                ...
                // Then
                fixture.Log.Entries.ShouldContain(x => x.Message == "0 issue(s) were filtered because they don't contain line or column information.");
            }

            [Fact]
            public void Should_Filter_Issues_Without_Priority()
            {
                // Given
                using var tempSourceFile = new TemporarySourceFile("Testfiles.TestFile.txt");
                var filePath = tempSourceFile.FilePath;
                ...
                // Then
                fixture.Log.Entries.ShouldContain(x => x.Message == "1 issue(s) were filtered because they don't have a priority set.");
            }

            [Fact]
            public void Should_Not_Filter_Issues_With_Priority()
            {
                // Given
                using var tempSourceFile = new TemporarySourceFile("Testfiles.TestFile.txt");
                var filePath = tempSourceFile.FilePath;
                ...
                // Then
                fixture.Log.Entries.ShouldContain(x => x.Message == "0 issue(s) were filtered because they don't have a priority set.");
            }
